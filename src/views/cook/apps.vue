      ...
      <template v-for="(app,index) in routes">
        <van-col span="6" :key="index" v-if="!app.hidden">
          <div class="mu-paper" @click="$router.push({name:app.name})">
            <span class="app-icon" :style="{background:app.meta.background}">{{app.name}}</span>
...
export default {
  ...
  mounted() {
    request.get(API.DINGTALK_FOREST_GETUSERINFO).then(res => {
      window.sessionStorage.setItem('bamboo-userinfo', JSON.stringify(res.data ? (res.data.data || {}) : {}))
    }).catch(err => {
      console.error(err)
    })

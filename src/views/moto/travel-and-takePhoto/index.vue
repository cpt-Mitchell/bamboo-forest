    ...
    <div style="padding:0 10px;">
      <van-cell-group>
        ...
        <div class="van-cell van-field" style="color: rgb(51, 51, 51);">
          ...
          <div class="van-cell__value">
            <div class="van-field__body">
              <select type="text" class="van-field__control" v-model="form.rangeType">
        ...
        <van-field
          class="no-disabled_bgcolor"
          label="搭乘人员"
          placeholder="点击选择搭乘人员"
          v-model="applyUser.name"
          @click.native="userSelectorShowHandle"
          disabled
        />
        <van-field
          class="no-disabled_bgcolor"
  ...
  methods: {
    ...
    getUserResultHandle(result) {
      if (result && result.users && result.users.length > 0) {
        this.applyUser = result.users[0] || {}
      }
    },
    userSelectorShowHandle() {
    ...
    doSubmit() {
      let payload = Object.assign({}, this.form, {
        photo: this.photo,
        takerName: this.applyUser.name,
        takerNo: this.applyUser.id
      })
      let validMsg = ''

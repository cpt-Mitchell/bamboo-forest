<!-- 造林抚育作业进度汇报 -->
<template>
  <div class="aff-tend-progress-box">
...
export default {
  ...
  methods: {
    ...
    querySelector(params, nextArrKey) {
      ...
      request
        .post(API.FOREST_JOB_APPLY_FORSTATION_QUERY_SELECT_ITEMS, params)
        .then(res => {
    ...
    doSubmit() {
      this.$dConfirm('确定提交表单内容吗？', () => {
        ...
        request
          .post(API.FOREST_JOB_FORESTATION_SAVE, this.form)
          .then(res => {
            this.$loadingState(false)
            let errorCode = res.data.code
            dAlert(res.data.msg, () => {
  ...
  mounted() {
    ...
    this.$title('造林抚育作业进度汇报')
  }
}
</script>

<style lang="less">
.aff-tend-progress-box {

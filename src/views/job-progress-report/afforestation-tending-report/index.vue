    ...
    <div style="padding:0 10px;">
      <van-cell-group>
        ...
        <van-field
          ...
          disabled
        />
        <van-field type="number" v-model="form.area" label="作业量" placeholder="请填写作业量" />
      </van-cell-group>
    </div>
...
export default {
  ...
  methods: {
    ...
    querySelector(params, nextArrKey) {
      ...
      request
        .post(API.FOREST_JOB_QUANTITY_QUERY_SELECT_ITEMS, params)
        .then(res => {
    ...
    doSubmit() {
      this.$dConfirm('确定提交表单内容吗？', () => {
        ...
        request
          .post(API.FOREST_JOB_QUANTITY_SAVE, this.form)
          .then(res => {
            this.$loadingState(false)
            let errorCode = res.data.code
            dAlert(res.data.msg, () => {

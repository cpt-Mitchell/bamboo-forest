    ...
    <div style="padding:0 10px;">
      <van-cell-group>
        ...
        <van-field
          ...
          @blur="filterInputSize(1, 'todayJoinPerson')"
        />
        <van-field
          type="number"
          v-model="form.stopDays"
          label="停工天数"
          placeholder="请输入停工天数"
          @blur="filterInputSize(2, 'stopDays')"
        />
        <div class="van-cell van-field" style="color: rgb(51, 51, 51);">
          <div class="van-cell__title">
            <span>停工原因</span>
          ...
          <div class="van-cell__value">
            <div class="van-field__body">
              <select class="van-field__control" v-model="form.stopReason">
                <option :value="null" selected>请选择停工原因</option>
                <option
                  v-for="(option, index) in ['天气', '纠纷', '公司原因', '物资供应不及时']"
                  :key="index"
                  :value="option"
                  >{{ option }}</option
                >
...
export default {
  ...
  methods: {
    ...
    querySelector(params, nextArrKey) {
      ...
      request.post(API.FOREST_JOB_FORESTATION_STOP_QUERY_SELECT_ITEMS, params).then(res => {
    ...
    doSubmit() {
      this.$dConfirm('确定提交表单内容吗？', () => {
        ...
        request
          .post(API.FOREST_JOB_FORESTATION_STOP_SAVE, this.form)
          .then(res => {
            this.$loadingState(false)
            let errorCode = res.data.code
            dAlert(res.data.msg, () => {

  ...
  <div class="logging-progress-report-box">
    <div style="padding:0 10px;">
      <van-cell-group>
        ...
        <van-field
          type="number"
          v-model="form.todayJoinRealCar"
          label="当天实际作业采伐机台数"
          placeholder="请输入实际作业采伐机台数"
          @blur="filterInputSize(0, 'todayJoinRealCar')"
        />
        <van-field
          type="number"
          v-model="form.todayJoinTotalTime"
          label="当天采伐机实际作业总时间"
          placeholder="请输入采伐机实际作业总时间"
          @blur="filterInputSize(3, 'todayJoinTotalTime')"
        />
        <van-field
          type="number"
...
export default {
  ...
  methods: {
    ...
    querySelector(params, nextArrKey) {
      ...
      request.post(API.FOREST_JOB_MACHINERY_QUERY_SELECT_ITEMS, params).then(res => {
    ...
    doSubmit() {
      ...
      if (Number.parseFloat(this.form.transportMainWood || 0) === 0 && !reason) {
      ...
      this.$dConfirm('确定提交表单内容吗？', () => {
        ...
        request
          .post(API.FOREST_JOB_MACHINERY_SAVE, this.form)
          .then(res => {
            this.$loadingState(false)
            let errorCode = res.data.code
            dAlert(res.data.msg, () => {
